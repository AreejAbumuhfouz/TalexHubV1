...
// const { User, OtpToken, RefreshToken, Wallet } = require('../models');
...
//   res.cookie('refreshToken', refreshToken, { ...base, maxAge: rememberMe ? 30 * 24 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000 });
// };
...
//   where: {
//     userId,
//     token: hashOtp(otp),  // ← compare against hash
//     purpose,
//     used: false,
...
//     // أعطِ المُحيل نقاطه (fire-and-forget — لا نوقف التدفق لو فشل)
...
//   await OtpToken.create({
//     userId,
//     token: hashOtp(otp),  // ← مشفّر دائماً
//     purpose,
//     expiresAt: new Date(Date.now() + 10 * 60 * 1000),
//   });
...
//   // التحقق من حالة الحساب بعد التأكد من صحة كلمة المرور
...
// /* ════════════════════════════════════════════════════════════
//    GOOGLE CALLBACK
//    Redirect to /auth/callback so the frontend can call checkAuth()
//    and hydrate the Zustand store before navigating to /dashboard.
// ════════════════════════════════════════════════════════════ */
...
//     // Redirect to /auth/callback — NOT /dashboard directly
...
//     where: {
//       userId,
//       token: hashOtp(otp),  // ← مقارنة بالهاش دائماً
//       purpose: 'password_reset',
//       used: false,
//       expiresAt: { [Op.gt]: new Date() },
//     },
...
//   // Check if plan has expired since last login
...
//         pointsDelta: -oldPoints, cashDelta: 0,
...
exports.login = async (req, res) => {
  ...
  // لو دور المستخدم company → نجيب status الشركة لاستخدامه في الـ redirect
  ...
  return success(res, {
    ...
    companyStatus, // 'active' | 'pending_review' | 'pending_documents' | 'rejected' | 'suspended' | null
  }, 'تم تسجيل الدخول بنجاح');
};
